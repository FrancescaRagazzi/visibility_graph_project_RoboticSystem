...
class Cart2DRobot(RoboticSystem):
    ...
    def __init__(self, x, y, xT, yT):
        ...
        self.trajectory = StraightLine2DMotion(0.2, 0.5, 0.5) # velocità max, acc e dec
        (x, y, _) = self.get_pose() #get pose titorna x y e angolo ma per avviare il moto ci interessa solo x e y
        self.trajectory.start_motion((x, y), (xT, yT)) #x e y la posa e xT e yT la pos target

        self.plotter = DataPlotter()

    def run(self):
        (x_target, y_target) = self.trajectory.evaluate(self.delta_t) #prendiamo il target dal robot virtuale
...
if __name__ == '__main__':
    cart_robot = Cart2DRobot(0,0,0.5,0.5)
